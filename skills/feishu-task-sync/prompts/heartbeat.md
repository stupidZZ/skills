...
| 跳过：非指派给我 | `latest-report.json.skipped_wrong_assignee_count`（0.3.15+；例如 third_party_assignee / cc_only）|
...
### 5a. 非指派给我的行动项（0.3.15+，仅在 `skipped_wrong_assignee_count > 0` 时出现）

如果 Agent 识别到一条“看起来像行动项”的消息，但 `assignee_evidence` 是
`third_party_assignee` / `cc_only` / `no_assignee` / `ambiguous_assignee`，
`feishu_tasks.py create` 会硬拦并记入 `skipped_wrong_assignee_count`。

心跳里简短列出最多 5 条：标题、assignee_evidence、原文里实际执行人/cc（如果 description 有写）。
表达口径：

> 识别到 N 条行动项但不是指派给我，已跳过；不会创建到我的飞书任务列表。

典型例子：`今天涨涨在数据需求群里找计川分析… cc@Zake` → 执行人是涨涨，cc 是 Zake，不是 ZZ，因此跳过。

### 5b. 话题 / thread 采集状态（0.3.10+，每轮都简短显示）
...
### 5c. 任务可见性预警（仅在 `created_but_invisible_count > 0` 或 `visibility_unknown_count > 0` 时出现）
