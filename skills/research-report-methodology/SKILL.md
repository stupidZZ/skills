...
description: |
  ...
  baseline pairing, interpretable tables, metric definitions, and bounded
  conclusions.
version: 0.1.0
...
2. Check whether the section has the baseline needed to support that claim.
3. Redesign the table so identity columns come first, variable columns second,
   and metrics last.
4. Add or tighten the setting block.
5. Add glossary entries for unclear terms and internal labels.
6. Rewrite conclusions to include observation, interpretation, and boundary.
7. Validate that the report is readable without opening source configs,
   scripts, or data files.
8. If the report is generated by code, run a render or row/column count check.
